            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">特产收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="techanName" id="techanName" placeholder="特产名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">特产类型</label>
                              <div class="layui-input-inline">
                                  <select name="techanTypes" id="techanTypes" lay-filter="techanTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in techanTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../techan/detail.jsp?id='+item.techanId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.techanPhoto?item.techanPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../techan/detail.jsp?id='+item.techanId)"><span>{{item.techanName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.techanNewMoney"><span style="color: red;">{{item.techanNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteTechanCollection(item.id)">删除 {{item.techanName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 techanCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            techanCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteTechanCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('techanCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#techanName').val() != null && jquery('#techanName').val() != "" && jquery('#techanName').val() != "null" ) {
                param['techanName'] = jquery('#techanName').val();
            }
                     
            var techanTypes = $("#techanTypes").find("option:selected").val();
            if (techanTypes != null && techanTypes != "" && techanTypes != "null") {
                            param['techanTypes'] = techanTypes;
                        }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('techanCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function techanTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=techan_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.techanTypesList = res.data.list;
               }
           });
       }
