            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑特产留言</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6 aaaaaa">
                                        <label>特产</label>
                                        <div>
                                            <select id="techanSelect" name="techanSelect"
...
<script>
    ...
    function upload() {
        ...
        $('#techanLiuyanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('techanLiuyanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            getContent();
           if($("#techanId") !=null){
               var techanId = $("#techanId").val();
               if(techanId == null || techanId =='' || techanId == 'null'){
                   alert("特产id不能为空");
                   return;
               }
           }
           if($("#yonghuId") !=null){
            ...
            httpJson("techanLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        <!-- 查询级联表的所有列表 -->
        function techanSelect() {
            //填充下拉框选项
            http("techan/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    techanOptions = res.data.list;
                }
            });
        }

        function techanSelectOne(id) {
            http("techan/info/"+id, "GET", {}, (res) => {
                if(res.code == 0){
                ruleForm = res.data;
                techanShowImg();
                techanShowVideo();
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationtechanSelect() {
            var techanSelect = document.getElementById('techanSelect');
            if(techanSelect != null && techanOptions != null  && techanOptions.length > 0 ) {
                for (var i = 0; i < techanOptions.length; i++) {
                        techanSelect.add(new Option(techanOptions[i].techanName, techanOptions[i].id));
                }

                $("#techanSelect").change(function(e) {
                        techanSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(techanSelect != null && techanOptions != null  && techanOptions.length > 0 ) {
                for (var i = 0; i < techanOptions.length; i++) {
                    if (techanOptions[i].id == ruleForm.techanId) {//下拉框value对比,如果一致就赋值汉字
                        techanSelect.options[i+1].selected = true;
                        $("#techanSelect" ).selectpicker('refresh');
                    }
                }
            }
            var yonghuSelect = document.getElementById("yonghuSelect");
    ...
    function setContent() {
        ...
        if (ruleForm.techanLiuyanContent != null && ruleForm.techanLiuyanContent != 'null' && ruleForm.techanLiuyanContent != '') {
            ...
            techanLiuyanContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.techanLiuyanContent != null){
                    mes = ''+ ruleForm.techanLiuyanContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
            var techanLiuyanContentEditor = UE.getEditor('techanLiuyanContentEditor');
            if (techanLiuyanContentEditor.hasContents()) {
                $('#techanLiuyanContent-input').attr('value', techanLiuyanContentEditor.getPlainTxt());
            }
    ...
    // 获取当前详情
    function getDetails() {
        var addtechanLiuyan = window.sessionStorage.getItem("addtechanLiuyan");
        if (addtechanLiuyan != null && addtechanLiuyan != "" && addtechanLiuyan != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("techanLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#techanId").val(ruleForm.techanId);
        $("#yonghuId").val(ruleForm.yonghuId);
        $("#techanLiuyanContent").val(ruleForm.techanLiuyanContent);
        $("#replyContent").val(ruleForm.replyContent);
        $("#insertTime-input").val(ruleForm.insertTime);

    }
    <!--  级联表的数据回显  -->
    function techanDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#techanId").val(ruleForm.id);

        $("#techanName").val(ruleForm.techanName);
        $("#techanValue").val(ruleForm.techanValue);
    ...
    function techanShowVideo() {
        $("#techanPhotoV").attr("src",ruleForm.techanPhoto);
    }
    ...
    function readonly() {
        ...
        if(window.sessionStorage.getItem('addtechanLiuyan') != null && window.sessionStorage.getItem('addtechanLiuyan') !=""){//新增查询,修改不查
            // 查询当前登录账户的信息
            var userId = window.sessionStorage.getItem('userId');
